...
class PickFolderHelpersTest(unittest.TestCase):
    ...
    def test_macos_path_containing_spaces(self):
        ...
        self.assertFalse(result["cancelled"])
        self.assertIsNone(result["error"])
        # Prompt is an argv item (last arg), not interpolated into -e source.
        args = run.call_args[0][0]
        self.assertEqual(args[0], "osascript")
        self.assertEqual(args[-1], "Pick models folder")
        self.assertTrue(any("choose folder" in a for a in args))
        self.assertNotIn('with prompt "Pick models folder"', " ".join(args))
...
class BrowseFolderApiSafeTest(unittest.TestCase):
    ...
    def test_api_rejects_non_loopback(self):
        bridge._pick_folder_impl = lambda title: bridge._picker_ok("/tmp/should-not-run")
        sent = {}

        class FakeHandler(bridge.Handler):
            def __init__(self):
                # Tailscale CGNAT peer — picker must refuse (paste path instead).
                self.client_address = ("100.64.1.2", 0)

            def _read_json(self):
                return {"title": "Pick a folder"}
        ...
        FakeHandler()._handle_api_post("/api/browse-folder", None)
        self.assertEqual(sent["status"], 403)
        self.assertEqual(sent["body"]["code"], "picker_local_only")
        self.assertIn("Paste the folder path", sent["body"]["message"])
